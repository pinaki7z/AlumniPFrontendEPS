...
const Jobs = () => {
  ...
  const getData = async () => {
    try {
      ...
      const filteredMyJobs = response.data.filter(
        (job) => job.userId === profile._id && !job.archive && job.approved
      );
      ...
      <div className="flex flex-col w-full">
        ...
        <Routes>
          <Route
            path="/"
            element={
              <div className="lg:px-10 px-4 z-10">
                <PageSubTitle
                  ...
                  buttontext4Link={buttontext4Link}
                  buttontext5Link={buttontext5Link}
                  name="jobs"
                  create={false}
                />
              </div>
        ...
        <Routes>
          <Route
            ...
            element={
              <div className="job-poztt">
                <div className="flex flex-wrap gap-5vw pt-5">
                  {loading ? (
                    <div>Loading.....</div>
                    ...
                    filteredMyJobs.map((job) => (
                      <div key={job._id} className="job-post">
                        <JobPost
                          ...
                          duration={job.duration}
                          jobType={
                            job.jobType ? job.jobType : job.internshipType
                          }
                          questions={job.questions}
                          category={job.category}
                          currency={job.currency}
          ...
          <Route
            path="/"
            element={
              <div className="job-poztt px-20">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 p-5 ">
                  {loading ? (
                    <div>Loading.....</div>
                  ) : filteredJobs.length ? (
                    filteredJobs.map((job) => (
                      <div key={job._id} className=" h-full w-full">
                        <JobPost
                          userId={job.userId}
                          id={job._id}
                          jobTitle={job.title}
